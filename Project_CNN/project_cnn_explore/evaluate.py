...
def main(not_parsed_args):
    ...
    if (FLAGS.frozenInference):
        ...
        if FLAGS.arch_type == "dcscn":
        ...
        elif FLAGS.arch_type == "v3_edge_concat":
            model.build_graph_v3_edge_concat()   
            ...
            model.build_graph_v7_1_edge_concat()              
        elif FLAGS.arch_type == "v8_edge_concat":
            model.build_graph_v8_edge_concat()  
        elif FLAGS.arch_type == "v8_1_edge_concat":
            model.build_graph_v8_1_edge_concat()              
    ...
    if FLAGS.test_dataset == "all":
        test_list = ['set5', 'set14', 'bsd100', 'Urban100', 'div2k_sample']
    else:
...
def evaluate_bicubic(model, test_data):
    ...
    for filename in test_filenames:
        psnr, ssim, psnr_rgb, ssim_rgb = model.evaluate_bicubic(filename, print_console=False)
        total_psnr += psnr
        total_ssim += ssim
